...
def test_heston_model(instrument: Instrument, n_paths: int = 1000, horizon_bars: int = 60):
    """Test Heston model for a given instrument."""
    logger.info(f"\n{'='*70}")
    logger.info(f"Testing Heston Model: {instrument.value}")
    logger.info(f"{'='*70}")
    ...
    recent_returns = returns[-1000:]  # Use recent data for fitting
    
    # Fit Heston model
    logger.info("Fitting Heston model...")
    dt = 1.0 / (252 * 390)  # 1 minute in years
    
    try:
        heston_model = HestonModel()
        heston_params = heston_model.fit(recent_returns, dt=dt)
        
        logger.info(f"Heston parameters:")
        logger.info(f"  κ (kappa): {heston_params.kappa:.4f}")
        logger.info(f"  θ (theta): {heston_params.theta:.8f} (per-step)")
        logger.info(f"  ξ (xi): {heston_params.xi:.4f}")
        logger.info(f"  ρ (rho): {heston_params.rho:.4f}")
        logger.info(f"  μ (mu): {heston_params.mu:.8f}")
        logger.info(f"  Feller satisfied: {heston_params.feller_satisfied}")
        
        # Generate paths
        logger.info("Generating paths...")
        initial_price = float(df['close'].iloc[-1])
        initial_variance = float(np.var(recent_returns[-100:]))
        initial_variance = max(initial_variance, 1e-8)
        
        prices, variances = heston_model.simulate_paths(
            n_paths=n_paths,
        ...
        logger.info(f"\nStylized Facts (Heston Model):")
        logger.info(f"  Kurtosis: {facts.get('kurtosis', 0):.3f} (target: >3 for fat tails)")
        ...
        logger.error(f"Error testing Heston model: {e}", exc_info=True)
        return None


def main():
    """Run Heston model tests for all instruments."""
    instruments = [Instrument.ES, Instrument.NQ, Instrument.GC, Instrument.SI]
    
    results = {}
    for instrument in instruments:
        result = test_heston_model(instrument, n_paths=1000, horizon_bars=60)
        if result:
